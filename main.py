...
apoluyanov_room = work_client.join_room(config.apoluyanov)
...
def on_message(room, event):
    ...
    and 'Service messange:' not in event['content']['body']:
        ...
        #если мы не поняли че нам прислали
...
#apoluyanov_room.add_listener(on_message) # тестовая комната с саппортом
work_client.start_listener_thread()  # запускаем тред слушателя

apoluyanov_room.send_text('Service messange: Bot started.')
